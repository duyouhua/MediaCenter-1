...
public class LrcView extends TextView {
	...
	protected void onDraw(Canvas canvas) {
		...
		try {
			...
			canvas.drawText("正在搜索歌词", width / 2,
					high / 2, CurrentPaint);
		}
	}
	
	public void setNonePlay(){
		LrcContent lrcContext=new LrcContent();	
		lrcContext.setLrc("无歌曲播放");
		lrcContext.setLrc_time(0);
		mSentenceEntities=new ArrayList<LrcContent>();
		mSentenceEntities.add(lrcContext);
		Index=0;
	}
	
	public void setSearchingLrc(){
		LrcContent lrcContext=new LrcContent();	
		lrcContext.setLrc("正在搜索歌词");
		lrcContext.setLrc_time(0);
		mSentenceEntities=new ArrayList<LrcContent>();
		mSentenceEntities.add(lrcContext);
		Index=0;
